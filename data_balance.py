...
def generate_full_data(sample_size,realization):
    ...
    for i in range(sample_size):
        [X, t, yf, ycf] = gen1([10,10,10],realization,weight_mu,weight_sigma )
        sample_x.append(X)
        sample_t.append(t)
        sample_yf.append(yf)
...
#print q[2]
np.savez('./data/synthetic_train_balance_%d.npz'%realization, x=q[0], t= q[1], yf=q[2], ycf=q[3], mu0=q[4],
        mu1=q[5], ate=q[6], yadd=q[7], ymul=q[8])
# print sss
# sss = 0
np.random.seed(1)
q = generate_full_data(1000,realization)
np.savez('./data/synthetic_test_balance_%d.npz'%realization, x=q[0], t= q[1], yf=q[2], ycf=q[3], mu0=q[4],
        mu1=q[5], ate=q[6], yadd=q[7], ymul=q[8])
# print sss
